...
// Fraud Check model
const FraudCheck = sequelize.define(
  "FraudCheck",
  {
    checkId: {
      type: DataTypes.UUID,
...
// Fraud Rule model
const FraudRule = sequelize.define(
  "FraudRule",
  {
    ruleId: {
      type: DataTypes.UUID,
...
const CustomerRiskProfile = sequelize.define(
  "CustomerRiskProfile",
  {
    profileId: {
      type: DataTypes.UUID,
...
CustomerRiskProfile.hasMany(FraudCheck, {
  foreignKey: "customerId",
  sourceKey: "customerId",
});
FraudCheck.belongsTo(CustomerRiskProfile, {
  foreignKey: "customerId",
...
CustomerRiskProfile.prototype.updateRiskScore = async function (increment) {
  this.baseRiskScore = Math.min(
    100,
    Math.max(0, this.baseRiskScore + increment),
  );
